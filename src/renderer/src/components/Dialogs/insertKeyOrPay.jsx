...
function InsertKeyOrPay({show,res}) {
  ...
  useEffect(()=>{
    ...
    setItems([
        {name:t('common.i-have-a-key'),id:1,msg:t('messages.i-want-to-buy-key-msg')},
        {name:t('common.i-want-to-buy-key'),id:2,msg:t('messages.i-want-to-buy-key-msg')}
    ...
    <div className={`relative p-4 w-full max-w-md max-h-full duration-150 ease-in-out  ${!show ? ' translate-y-4' :''}`}>
        <div className="relative bg-white rounded-lg shadow">
            ...
            <div className="p-4 md:p-5">
                <p className="text-gray-500 mb-4">{t('common.select')}:</p>
                <ul className="space-y-4 mb-4 max-h-[300px] overflow-auto">
                   
                    
                   {items.map((i,_i)=>(
                        <li onClick={()=>setSelected(i.id)} key={_i}>
                        ...
                        <label for={`job-${_i}`} className="inline-flex items-center justify-between w-full p-5 text-gray-900 bg-white border border-gray-200 rounded-lg cursor-pointer peer-checked:border-app_orange-300 peer-checked:text-app_orange-300 hover:text-gray-900 hover:bg-gray-100">
                            <div className="block">
                                ...
                                <div className="w-full text-gray-500 text-[14px]">{i.msg}</div>
                            </div>
                ...
                <button onClick={()=>{
                      res(selected)
                }} className={`text-white inline-flex w-full justify-center  focus:outline-none ${selected!=null ? 'focus:ring-app_orange-300 bg-app_orange-500 hover:bg-app_orange-400 focus:ring-4' :' bg-gray-400 opacity-70 cursor-not-allowed'} font-medium rounded-lg text-sm px-5 py-2.5 text-center`}>
                      {t('common.confirm')}
                </button>
