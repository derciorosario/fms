    ...
    <div className={`relative p-4 w-full max-w-md max-h-full duration-150 ease-in-out  ${!show ? ' translate-y-4' :''}`}>
        <div className="relative bg-white rounded-lg shadow">
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                ...
                <button onClick={()=>res(false)} type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm h-8 w-8 ms-auto inline-flex justify-center items-center" data-modal-toggle="select-modal">
            ...
            <div className="p-4 md:p-5">
                ...
                <ul className="space-y-4 mb-4">
                   ...
                   {items.map((i,_i)=>(
                        ...
                        <label for={`job-${_i}`} className="inline-flex items-center justify-between w-full p-5 text-gray-900 bg-white border border-gray-200 rounded-lg cursor-pointer dark:peer-checked:text-app_orange-300 peer-checked:border-app_orange-300 peer-checked:text-app_orange-300 hover:text-gray-900 hover:bg-gray-100">
                            <div className="block">
                                <div className="w-full text-lg font-semibold">{i.name}</div>
                                <div className="w-full text-gray-500 dark:text-gray-400">{i.is_admin ? 'Administrador' : 'Gestor'}</div>
