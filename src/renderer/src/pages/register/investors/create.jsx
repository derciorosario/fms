       ...
       function App() {
         ...
         const {navigate}=useNavigate()
          ...
          useEffect(()=>{
            ...
            (async()=>{
              try {
                let item=await db.investors.get(id)
                setFormData(item)
              } catch (error) {
                console.log(error)
              }
          ...
          useEffect(()=>{
            (async()=>{
                let docs=await db.investors.allDocs({ include_docs: true })
         ...
         async function SubmitForm(){
              ...
              if(valid){
                  if(items.some(i=>i.email==formData.email && i._id!=id && i.email) || items.some(i=>i.name?.toLowerCase()==formData.name?.toLowerCase() && i._id!=id && i.last_name?.toLowerCase()==formData.last_name?.toLowerCase())){
                   ...
                   try{
                     if(id){
                        _update('investors',[{...formData}])
                        toast.success('Fornecedor actualizado')
                     }else{
                        _add('investors',[{...formData,id:Math.random(),_id:Math.random().toString()}])
                        setVerifiedInputs([])
         ...
         return (
           <>
              <DefaultLayout details={{name: (id ?'Actualizar' :'Novo')+' Investidor'}} >
