...
export default function Table({itemsToDelete,setItemsToDelete}) {
       const {_investors,_get,_loaded}= useData()
       ...
       const columns = [
            ...
            
            {
                ...
                renderCell: (params) => (
                   <div style={{opacity:.8}}>
                        <span style={{marginRight:'0.5rem',cursor:'pointer'}} onClick={()=>navigate('/supplier/'+params.row._id)}>
                            <EditOutlinedIcon/>
                        </span>
      ...
      const [rows,setRows]=React.useState(_investors)
      ...
      return (
        <Box sx={{ height:'400px', width: '100%' }}>
          <DataGrid
            ...
            localeText={{ noRowsLabel: <TableLoader loading={!_loaded.includes('investors') ? true : false}/>}}
          />
        </Box>
      );
    }
