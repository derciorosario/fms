...
export default function Table({itemsToDelete,setItemsToDelete}) {
       const {_managers,_get,_loaded}= useData()
       ...
       const columns = [
            ...
            
            {
                ...
                renderCell: (params) => (
                   <div style={{opacity:.8}}>
                        <span style={{marginRight:'1rem',cursor:'pointer'}} onClick={()=>navigate('/manager/'+params.row._id)}>
                            <EditOutlinedIcon/>
                        </span>
      ...
      const [rows,setRows]=React.useState(_managers)
      ...
      return (
        <Box sx={{ height:'400px', width: '100%' }}>
          <DataGrid
            ...
            localeText={{ noRowsLabel: <TableLoader loading={!_loaded.includes('managers') ? true : false}/>}}
          />
        </Box>
      );
    }
