...
function App() {
  ...
  const [dataCatPer,setCatPer]=useState({labels:[],datasets:[]})
  const [weekBalance,setWeekBalance]=useState(0)
  const navigate = useNavigate()


  useEffect(()=>{
    const {datasets:d_cm,labels:l_cm} = _get_cash_managment_stats([filterOptions.filter(i=>i.id=="monthy_cm")[0]],'w')
    setDataChartCM({datasets:d_cm,labels:l_cm})
    if(!(required_data.some(i=>!_loaded.includes(i)))){
        setInitialized(true)
    }

    setWeekBalance(_get_stat('this_week_transations').balance)
    setCatPer(_get_stat('monthly_cat_performace',{period:"m"}).barChart)
  },[_loaded,filterOptions,i18n.language])
...
<div className="p-6 flex justify-center">
  ...
  <div className="flex mb-5 mt-3 max-lg:flex-col">
                ...
                <div className="lg:max-w-sm w-full bg-white rounded-[0.3rem] shadow-sm  p-4 md:p-6 max-lg:mb-3">
                  <div className="flex justify-between border-gray-200 border-b  pb-3">
                    <div>
                      ...
                      <div className={`leading-none text-3xl font-bold ${weekBalance < 0 ? 'text-red-600' :'text-gray-900'}`}>{weekBalance < 0 ? `(${_cn(weekBalance).replaceAll('-','')})` : _cn(weekBalance)}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 py-3">
                    <dl>
                      ...
                      <dd className="leading-none  font-bold text-green-500">{_cn(_get_stat('this_week_transations').inflows)}</dd>
                    </dl>
                    <dl>
                      ...
                      <dd className="leading-none  font-bold text-red-600">{_cn(_get_stat('this_week_transations').outflows).replaceAll('-','')}</dd>
                    </dl>
                    
                    <dl>
                      <dt className="text-base font-normal text-gray-500 pb-1">{t('common.initial-value')}</dt>
                      <dd className="leading-none font-bold text-orange-400">{_cn(_get_stat('this_week_transations').initial_amount).replaceAll('-','')}</dd>
                    </dl>
                  </div>
                
                  <div id="bar-chart">
                
                  <Doughnut {..._get_stat('this_week_transations').doughnut} />
                ...
                <div class="mb-3 p-2 flex flex-col rounded-[0.3rem] shadow-sm bg-white flex-1 ml-2">
                    ...
                    <div className="relative">
                        <ShowBalanceDetails full={true} show={true}/>
                    </div>
                </div>
  ...
  <div className="relative flex-1 bg-white shadow-sm rounded-[0.3rem] mb-2">
                
                <div className="px-3 py-2 flex justify-between items-center border-b">
                  <span className="font-semibold">{t('common.weekly-cash-managment')} {filterOptions.filter(i=>i.id=="monthy_cm")[0].groups[0].items[filterOptions.filter(i=>i.id=="monthy_cm")[0].groups[0].selected_ids[0] - 1].name?.toLowerCase()}</span>
...
<div key={_i} class={`${_i==0 ? 'lg:w-[48.8%]':'lg:w-[50%]'} w-full  mb-3 rounded-[0.3rem] shadow-sm bg-white ${_i==0 ? 'lg:mr-2':''}`}>
  ...
  <div class="py-6 relative" id="donut-chart">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" height="140px" viewBox="0 -960 960 960" fill="rgba(107,114,128 , 0.09)"><path d="M441-82Q287-97 184-211T81-480q0-155 103-269t257-129v120q-104 14-172 93t-68 185q0 106 68 185t172 93v120Zm80 0v-120q94-12 159-78t79-160h120q-14 143-114.5 243.5T521-82Zm238-438q-14-94-79-160t-159-78v-120q143 14 243.5 114.5T879-520H759Z"/></svg>
       ...
       {i=="out" &&  <div className="mx-5">
           <MixedChart {..._get_stat('cat_performace').barChart_out}/>
       </div>}

      {i=="in" && <div>
           <Doughnut {..._get_stat('cat_performace').doughnut_in}/>
      </div>}
