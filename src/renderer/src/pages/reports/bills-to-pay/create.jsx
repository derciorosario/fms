...
import moment from 'moment';
       ...
       function App() {
         ...
         const calculateDaysLeft = (futureDateString) => {
          ...
          useEffect(()=>{
                ...
                (async()=>{
                try {
                    let item=await db.bills_to_pay.get(id)
                    setFormData({...item,
          ...
          useEffect(()=>{
            setAccountCategories(_account_categories.filter(i=>['supplier','others','state','expenses'].includes(i.account_origin)))
          },[_account_categories])
           ...
           useEffect(()=>{
            if(formData.account_origin=="supplier"){
                setReferenceOptions(_suppliers)
            }else{
                setReferenceOptions([])
            }
           },[formData.reference,_suppliers])

           useEffect(()=>{
            (async()=>{
                let docs=await db.bills_to_pay.allDocs({ include_docs: true })
         ...
         async function SubmitForm(){
              ...
              if(valid){
                   try{
                     if(id){
                        _update('bills_to_pay',[{...formData}])
                        toast.success('Conta actualizada')
                     }else{
                        _add('bills_to_pay',[{...formData,
          ...
          useEffect(()=>{
               ...
               if(!formData.reference.name && formData.account_origin=="supplier") {
                v=false
               }
               setValid(v)
          },[formData])
  ...
  return (
    <>
       <InstallentsDialog page={'pay'} show={showInstallments} items={formData.installments} formData={formData} setShow={setShowInstallments} setFormData={setFormData}/>
       <DefaultLayout details={{name:'Nova conta a pagar'}}>
               ...
               <div className="[&>_div]:border mb-3  [&>_div]:border-[#D9D9D9] flex items-center px-[1rem] [&>_div]:rounded-[0.4rem] [&>_div]:min-w-[110px] [&>_div]:mr-[10px]  justify-start">
                             ...
                             <div className={`items-center justify-center px-3 py-2 flex`}>
                                <span className="text-[15px] text-[#A3AED0] mr-2">Pago</span>
              ...
              />   
               ...
               <div>
                 <FormControl sx={{ m: 1, width: '100%',margin:0,height:40 }} size="small">
                                ...
                                label="Tipo de conta"
                                onChange={(e)=>{
                                      setFormData({...formData,account_origin:e.target.value,reference:e.target.value!="supplier" ? {id:null,name:null} : formData.reference})
                                }}
                ...
                <div>
                    <Autocomplete size="small"
                      value={formData.reference.name && formData.account_origin=="supplier" ? formData.reference.name : null}
                      onChange={(event, newValue) => {
                      ...
                      defaultValue={null}
                      inputValue={(formData.account_origin=="supplier") ? formData.reference.name  : "" }
                      onInputChange={(event, newInputValue) => {
                      ...
                      sx={{ width: 300 }}
                      disabled={formData.account_origin=="supplier" && !id ? false : true}
                      renderInput={(params) => <TextField {...params}

                      onBlur={()=>validate_feild('reference')}
                      error={(!formData.reference.name) && verifiedInputs.includes('reference') && formData.account_origin=="supplier" ? true : false}
                      helperText={(!formData.reference.name) && verifiedInputs.includes('reference') && formData.account_origin=="supplier" ? "Insira nome do fornecedor" :''}
                      
                       value={formData.reference.name} label="Referênciar fornecedor" />}
                    />   
                  </div>
               ...
               <div>
                        <FormControl sx={{ m: 1, width: '100%',margin:0,height:40 }} size="small">
                                ...
                                <InputLabel style={{margin:0,height:40}} id="demo-simple-select-error">Modo de pagamento</InputLabel>
                                <Select
                                labelId="demo-simple-select-error-label"
                                id="demo-simple-select-error"
