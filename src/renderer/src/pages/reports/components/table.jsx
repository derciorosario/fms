...
function StatsTable({content}) {
    ...
    let {period,data,showProjected,showIcons} = content
...
{/***   monthly   *** */}
{period=="m" && <table class={` ${period=="d" ? 'hidden' :''} ${showProjected==2 ? '_done' :''} ${showProjected==3 ? '_projected':''} _montly _table_stats w-full text-sm text-left rtl:text-right `}>
    ...
    <tbody>
        ...
        {data.map((i,_i)=>(
                       ...
                       <td onClick={()=>setOpenItems(openItems.includes('i-'+_i) ? openItems.filter(o=>o!='i-'+_i) : [...openItems,'i-'+_i])}  className={` flex items-center relative font-medium`} style={{color:i.color,cursor:i.sub ? 'pointer':'initial'}}><span className={`border-l-[3px] flex w-full absolute left-0 top-[-2px]`} style={{height:'calc(100% + 4px)',borderColor:i.color}}></span>
                        ...
                        {i.sub?.filter(x=>!x.disabled)?.length!=0  && i.sub && <div className={`${openItems.includes('i-'+_i) ? 'rotate-90':''}`}><KeyboardArrowRightIcon/></div>}
                       ...
                       {(i.sub?.length ? true : false) && (
                             ...
                             <>
                                {i.sub.filter(i=>!i.disabled).map((i2,_i2)=>(
                                    <>
